...
export const list = async (_: Request, res: Response) => {
  try {
    ...
    return res.send({ Status: 200, Data: admins });
  } catch (error) {
    console.error(error);
    return res.status(400).send({ Status: 500, Data: 'Internal Server Error' });
  }
...
export const deleteById = async (req: Request, res: Response) => {
  try {
    ...
    if (!admin) {
      return res.status(404).send({ Status: 400, Data: 'Admin not found' });
    }
    // Xóa admin
    await adminRepository.remove(admin);
    return res.send({ Status: 200, Data: `Admin id ${adminId} has been deleted.` });
  } catch (error) {
    console.error(error);
    return res.status(400).send({ Status: 500, Data: 'Internal Server Error' });
  }
